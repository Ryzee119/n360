...
void loop() {
  ...
  if (Xbox.XboxReceiverConnected) {
    for (uint8_t i = 0; i < 4; i++) {
      ...
      //if (Xbox.getButtonPress(L3, i))    btn_state[i] |= N64_XX  //Left stick press, uncomment and change assignment to use.
      //if (Xbox.getButtonPress(XBOX, i))  btn_state[i] |= N64_XX  //Centre 'XBOX' button, uncomment and change assignment to use.
      //if (Xbox.getButtonPress(Y, i))     btn_state[i] |= N64_XX  //Y button, uncomment and change assignment to use.
        ...
        if (Xbox.getButtonClick(XBOX, i) && Xbox.getButtonPress(L1, i) && Xbox.getButtonPress(R1, i)) {
          goldeneye_mode ^= 1;
          if(!goldeneye_mode){
            Xbox.setLedOn(LED1,0); //If goldeneye mode disabled, set led back to 1st quadrant.
          }
        }
        ...
        if (goldeneye_mode && i==0) {
          ...
          if (Xbox.getButtonPress(R2, 0))    btn_state[0] |= N64_Z; //Back right trigger will be a 'Z' press from player 1 controller and is 'FIRE'
          if (Xbox.getButtonPress(L2, 0))    btn_state[1] |= N64_Z; //Back left trigger will be a 'Z' press from player 2 controller and is 'AIM'
          ...
          apply_deadzone(&x1, &y1, Xbox.getAnalogHat(RightHatX, 0) / 32768.0, Xbox.getAnalogHat(RightHatY, 0) / 32768.0, DEADZONE_LOW, DEADZONE_HIGH);
          x1 *= 32768.0*0.90;
          y1 *= 32768.0*0.90;
          ...
          if (!Xbox.getButtonPress(R3, 0) && !Xbox.getButtonPress(L2, 0)) {
