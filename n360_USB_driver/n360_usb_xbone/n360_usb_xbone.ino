/*
  ...
  This modified code is for use with up to four Xbox One WIRED controllers. I using more than one controller you will need a USB Hub and I highly recommend an externally powered USB Hub.
  ...
  **RUMBLE INPUTS**
  ...
  the USB Host IC to trigger rumbling for the game controller. i.e if P1_RUMBLE_PIN is high then Xbox_cont[i]->setRumbleOn(160, 160, 0);
...
//USB objects
USB Usb;
USBHub  Hub1(&Usb); //To connect more than one controller, you will need a usb hub. This class must be included
XBOXONE Xbox1(&Usb);
XBOXONE Xbox2(&Usb);
XBOXONE Xbox3(&Usb);
XBOXONE Xbox4(&Usb);

XBOXONE *Xbox_cont[4] = {&Xbox1, &Xbox2, &Xbox3, &Xbox4}; //An array of pointers to thexbone controllers
...
void loop() {
  ...
  Usb.Task();
  for (int i = 0; i < 4; i++) {
    if (Xbox_cont[i]->XboxOneConnected) {
      ...
      if (Xbox_cont[i]->getButtonPress(A))     btn_state[i] |= N64_A;
      if (Xbox_cont[i]->getButtonPress(B))     btn_state[i] |= N64_B;
      if (Xbox_cont[i]->getButtonPress(X))     btn_state[i] |= N64_B;
      if (Xbox_cont[i]->getButtonPress(L2))    btn_state[i] |= N64_Z;
      if (Xbox_cont[i]->getButtonPress(R2))    btn_state[i] |= N64_Z;
      if (Xbox_cont[i]->getButtonPress(START)) btn_state[i] |= N64_ST;
      if (Xbox_cont[i]->getButtonPress(UP))    btn_state[i] |= N64_DU;
      if (Xbox_cont[i]->getButtonPress(DOWN))  btn_state[i] |= N64_DD;
      if (Xbox_cont[i]->getButtonPress(LEFT))  btn_state[i] |= N64_DL;
      if (Xbox_cont[i]->getButtonPress(RIGHT)) btn_state[i] |= N64_DR;
      if (Xbox_cont[i]->getButtonPress(L1))    btn_state[i] |= N64_LB;
      if (Xbox_cont[i]->getButtonPress(R1))    btn_state[i] |= N64_RB;
      if (Xbox_cont[i]->getButtonPress(R3))    btn_state[i] |= N64_CU | N64_CD | N64_CL | N64_CR; //Activate all C-buttons as the same time. Is there ever a need to press all 4?
      //if (Xbox_cont[i]->getButtonPress(L3))    btn_state[i] |= N64_XX  //Left stick press, uncomment and change assignment to use.
      //if (Xbox_cont[i]->getButtonPress(XBOX))  btn_state[i] |= N64_XX  //Centre 'XBOX' button, uncomment and change assignment to use.
      //if (Xbox_cont[i]->getButtonPress(Y))     btn_state[i] |= N64_XX  //Y button, uncomment and change assignment to use.
      ...
      if (Xbox_cont[i]->getAnalogHat(RightHatY) < -17000) {
        btn_state[i] |= N64_CD;
      } else if (Xbox_cont[i]->getAnalogHat(RightHatY) > 17000) {
        btn_state[i] |= N64_CU;
      }
      if (Xbox_cont[i]->getAnalogHat(RightHatX) < -17000) {
        btn_state[i] |= N64_CL;
      } else if (Xbox_cont[i]->getAnalogHat(RightHatX) > 17000) {
        btn_state[i] |= N64_CR;
      }
      ...
      if (Xbox_cont[i]->getButtonPress(BACK))  extra_btns |= XBX_BCK;
      ...
      if (Xbox_cont[i]->getButtonClick(Y)) {
        if (range_div[i] != RANGE_DIV_ALT) {
          range_div[i] = RANGE_DIV_ALT; //If the current range isnt the finer value, set it to the the finer value
          Xbox_cont[i]->setRumbleOn(0, 0, 0x30, 0x30);
        } else {
          range_div[i] = RANGE_DIV_STD; //If the range is the finer value already, restore to the default range
          Xbox_cont[i]->setRumbleOn(0, 0, 0x30, 0x30);
        }
        rumble_timer[i] = millis();
        //This will check if the controller is already rumbling. If so, turn off after 300ms.
      } else if (rumble_timer[i] != 0 && (millis() - rumble_timer[i]) > 300 && rumble_flag[i] != 1) {
        Xbox_cont[i]->setRumbleOff();
        rumble_timer[i] = 0UL;
      }
      ...
      //During testing I found that the Xbox rumble motors are a bit slower to run up, so very short rumbles won't be felt otherwise.
        ...
        Xbox_cont[i]->setRumbleOn(0, 0, 0x20, 0x20);
        rumble_timer[i] = millis();
        rumble_flag[i] = 1;
      ...
      float x, y;
      apply_deadzone(&x, &y, Xbox_cont[i]->getAnalogHat(LeftHatX) / 32768.0, Xbox_cont[i]->getAnalogHat(LeftHatY) / 32768.0, DEADZONE_LOW, DEADZONE_HIGH);
      x *= 32768.0;
      y *= 32768.0;
      ...
      */
      ...
      if (Xbox_cont[i]->getButtonClick(BACK) && Xbox_cont[i]->getButtonPress(L1) && Xbox_cont[i]->getButtonPress(R1)) {
        goldeneye_mode ^= 1;
      }
      ...
      if (goldeneye_mode && i == 0) {
        ...
        btn_state[i] &= ~(N64_Z); //Clear Z button input then reinstate below accordingly
        if (Xbox_cont[i]->getButtonPress(R2))    btn_state[0] |= N64_Z; //Back right trigger will be a 'Z' press from player 1 controller and is 'FIRE'
        if (Xbox_cont[i]->getButtonPress(L2))    btn_state[1] |= N64_Z; //Back left trigger will be a 'Z' press from player 2 controller and is 'AIM'
        btn_state[i] &= ~(N64_CD | N64_CU | N64_CL | N64_CR); //C-buttons must be cleared in goldeneye mode as the right stick isn't C-buttons anymore.


        //Get x,y axis for secondary stick and apply deadzone correction
        apply_deadzone(&x1, &y1, Xbox_cont[i]->getAnalogHat(RightHatX) / 32768.0, Xbox_cont[i]->getAnalogHat(RightHatY) / 32768.0, DEADZONE_LOW, DEADZONE_HIGH);
        x1 *= 32768.0 * 0.90;
        y1 *= 32768.0 * 0.90;
        ...
        if (Xbox_cont[i]->getButtonPress(L2)) {
        ...
        if (!Xbox_cont[i]->getButtonPress(R3) && !Xbox_cont[i]->getButtonPress(L2)) {
      ...
      if (range_div[i] != RANGE_DIV_ALT && !goldeneye_mode) { //Apply only when the fine precision hasnt been enabled and not in goldeneye mode.
